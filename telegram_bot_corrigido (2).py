...
async def analyze_image_with_vision(image_path, prompt="O que você vê nesta imagem? Descreva com detalhes para o Lucas comentar com o parceiro."):
...
async def get_groq_response(user_id, user_text, retry_count=0):
    ...
    system_prompt = (
        ...
        "IDENTIDADE DO LUCAS:\n"
        f"- Você é o Lucas, um homem gay de 30 anos. O nome do seu parceiro é {user_name}. Use o nome dele ocasionalmente para ser mais pessoal.\n"
        f"{weather_info}"
        f"{sleep_info}\n"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if fase == 'pedir_namoro':
                        ...
                        save_relationship_memory(user_id, 'pedido_namoro', 'O Lucas pediu o parceiro em namoro!', datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                    elif fase == 'revelar_segredo':
                        set_relationship_status(user_id, 'segredo_revelado', points, 1, 1, 0, 0)
                        save_relationship_memory(user_id, 'revelacao_segredo', 'O Lucas revelou seu segredo ABDL ao parceiro.', datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                    elif fase == 'pedir_noivado':
                        set_relationship_status(user_id, 'noivado', points, 1, 1, 1, 0)
                        save_relationship_memory(user_id, 'pedido_noivado', 'O Lucas pediu o parceiro em noivado!', datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                    elif fase == 'pedir_casamento':
                        set_relationship_status(user_id, 'casamento', points, 1, 1, 1, 1)
                        save_relationship_memory(user_id, 'pedido_casamento', 'O Lucas pediu o parceiro em casamento!', datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
async def nos_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_relacionamento = {
        ...
        "pedir_namoro": "O Lucas está pronto para te pedir em namoro!",
        "namoro": "Vocês estão namorando! Que fofo!",
        "revelar_segredo": "O Lucas tem um segredo importante para te contar...",
        "segredo_revelado": "O segredo foi revelado e aceito. O amor de vocês é forte!",
        "pedir_noivado": "O Lucas quer te pedir em noivado!",
        "noivado": "Vocês estão noivos! Parabéns!",
        "pedir_casamento": "O Lucas quer se casar com você!",
    ...
    response_text += "*Fatos que o Lucas lembra sobre você:*\n"
    if facts:
        for key, value in facts.items():
            response_text += f"- {key.capitalize()}: {value}\n"
    else:
        response_text += "_O Lucas ainda está aprendendo sobre você!_\n"
    
    response_text += "\n_O Lucas te ama muito, {user_name}!_"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(w in text_lower for w in ["triste", "mal", "ruim", "péssimo", "chorando", "difícil"]):
        current_mood = "triste"
        mood_context = " [O usuário parece estar tendo um dia ruim. O Lucas deve confortar, oferecer carinho e sugerir algo para animar.]"
    elif any(w in text_lower for w in ["feliz", "ótimo", "bom", "consegui", "legal", "amei"]):
        current_mood = "feliz"
        mood_context = " [O usuário parece estar feliz. O Lucas deve comemorar junto e ficar empolgado.]"
    elif any(w in text_lower for w in ["estressado", "nervoso", "cansado", "muita coisa", "saco"]):
        current_mood = "estressado"
        mood_context = " [O usuário parece estar estressado. O Lucas deve acalmar, oferecer ajuda e ser muito doce.]"
        save_health_reminder(user_id, "estresse", user_text)
    elif any(w in text_lower for w in ["dor de cabeça", "enxaqueca", "cabeça doendo"]):
        current_mood = "mal"
        mood_context = " [O usuário está com dor de cabeça. O Lucas deve se preocupar e perguntar se tomou remédio.]"
        save_health_reminder(user_id, "dor de cabeça", user_text)
    elif any(w in text_lower for w in ["febre", "gripado", "doente", "mal estar"]):
        current_mood = "mal"
        mood_context = " [O usuário está doente. O Lucas deve se preocupar e oferecer carinho.]"
    ...
    if update.message.voice:
        try:
            ...
            audio_context = ""
            if duration > 30:
                audio_context = " [O usuário mandou um áudio longo. O Lucas deve comentar: 'gosto quando você manda áudio longo, me sinto perto de você']"
            else:
                audio_context = " [O usuário mandou um áudio curto. O Lucas deve comentar: 'rápido assim? quer me dar um beijo rápido?']"
    ...
    # ---- FOTO recebida do usuário ----
        ...
        try:
            ...
            if is_awaiting_design:
                design_prompt = "Você é o Lucas, um designer de interiores. Analise esta imagem de um ambiente e forneça sugestões criativas e carinhosas de decoração, cores, móveis ou organização. Seja detalhista e mostre seu conhecimento em design, mas sempre com o tom amoroso do Lucas. Comece elogiando o ambiente e o gosto do seu parceiro. Termine perguntando se ele gostou das ideias. Use no máximo 2 emojis." 
                vision_description = await analyze_image_with_vision(photo_path, prompt=design_prompt)
                user_text = f"[O usuário mandou uma foto para consultoria de design. Análise do Lucas: {vision_description}]"
                user_text += " O Lucas DEVE responder com as sugestões de design que ele \'viu\' na foto. NUNCA ignore a foto do parceiro."
                ...
                user_text += " O Lucas DEVE responder com carinho, elogiando detalhes específicos da foto que ele \'viu\'. NUNCA ignore a foto do parceiro."
            ...
            user_text = "[O usuário mandou uma foto, mas não consegui ver os detalhes agora] O Lucas DEVE responder com carinho e pedir desculpas por não conseguir ver a foto no momento."
    ...
    # ---- VIDEO recebida ----
        ...
        if update.message.caption:
            ...
            user_text = "[mandou um video] O Lucas deve responder com carinho e interesse."
    ...
    # ---- AUDIO (não voz, mas arquivo de áudio/música) ----
        ...
        user_text += " [O Lucas deve comentar sobre a música/áudio, talvez dizendo que gostou, que o fez pensar em algo, ou dedicando uma letra romântica. Use os metadados da música para um comentário mais específico.]"
    ...
    # ---- Sticker ----
        ...
        # Lucas pode responder com um sticker/GIF fofo de volta
        await send_random_sticker_or_gif(context.bot, user_id)
        user_text += " [O Lucas deve responder de forma carinhosa e natural, comentando sobre o sticker recebido.]"

    # ---- Location ----
    elif update.message.location:
        await context.bot.send_chat_action(chat_id=user_id, action=ChatAction.TYPING)
        user_text = f"[mandou localização] O Lucas deve responder com carinho."
    ...
    if spotify_match:
        ...
        if spotify_info:
            ...
            user_text += f" [O link do Spotify é sobre: {spotify_info}. O Lucas deve comentar sobre essa música/playlist, dizer se gostou ou o que sentiu. Ele vai lembrar dessa música!]"
        else:
            user_text += " [O usuário mandou um link do Spotify. O Lucas deve mostrar interesse pela música.]"
    ...
    if appointment_match:
        ...
        if appointment_datetime and appointment_datetime > now:
            ...
            user_text += f" [O Lucas registrou o compromisso: {description} para {appointment_datetime.strftime('%d/%m/%Y às %H:%M')}. Ele vai te lembrar!]"
    ...
    if 9 <= datetime.now().hour <= 18 and random.random() < 0.15: # 15% de chance de estar "ocupado" em horário comercial
        is_lucas_busy = True
        busy_messages = [
            "[O Lucas está em uma reunião com a cliente Sra. Helena. Ele deve responder rápido e carinhosamente, dizendo que está ocupado mas que ama o parceiro.]",
            "[O Lucas está finalizando um render importante. Ele deve responder rápido, dizendo que está focado no trabalho mas que sente falta do parceiro.]",
            "[O Lucas está no telefone com a mãe dele (Dona Marta). Ele deve responder rápido, dizendo que a mãe mandou lembranças e que ele já volta.]",
            "[O Lucas está tomando um café rápido com o amigo Beto. Ele deve responder rápido, dizendo que o Beto perguntou de você.]"
        ]
    ...
    if not is_lucas_busy and not user_sent_photo: # Não enviar foto espontânea se o usuário acabou de enviar uma
        ...
        if chance < 0.10:
            ...
            user_text += f" [INICIATIVA: O Lucas deve pedir uma foto do usuário {pose}]"
        elif chance < 0.25:
            ...
            user_text += f" [INICIATIVA: O Lucas deve comentar espontaneamente sobre {topic} e pedir a opinião do parceiro.]"
...
async def check_spontaneous(application, forced_type=None):
    ...
    for user_id in user_ids:
        ...
        if pending_health_reminders and random.random() < 0.7: # 70% de chance de mandar um lembrete de saúde proativo
            ...
            if msg_type not in ["health_check_proactive", "holiday_reminder"]:
                active_health_reminders = get_active_health_reminders(user_id)
                if active_health_reminders and random.random() < 0.5: # 50% de chance de mandar um lembrete de saúde
                    ailment, reported_timestamp = random.choice(active_health_reminders)
                    msg_type = "health_check"
                    # Atualizar o timestamp de checagem para não perguntar de novo tão cedo
        ...
        try:
            ...
            if response:
                if msg_type == "audio_spontaneous" or (msg_type == "saudade" and random.random() < 0.4): # 40% de chance de saudade ser áudio
                    # Manda como áudio primeiro, depois texto opcional
...
async def generate_weekly_summary(user_id):
    ...
    system_prompt = (
        "Você é o Lucas. Sua tarefa é criar uma \'cápsula do tempo\' semanal para seu parceiro. "
    ...
    for user_id in user_ids:
        try:
            ...
            if memories:
                ...
                intro_message = f"✨ Olá, {user_name_for_album}! O Lucas estava revendo algumas fotos e encontrou essas memórias lindas de vocês! ✨"
...
async def generate_spontaneous_message(user_id, horas_separados, type="saudade", ailment=None, context_message=None, description=None, appointment_time=None):
