...
def quebrar_em_bolhas(text):
    ...
    if len(partes) == 1 and len(text) > 140:
        ...
        if len(frases) > 2:
            ...
            if atual:
                partes.append(" ".join(atual).strip())
    return [p for p in partes if p][:3]
...
async def send_humanized(bot, chat_id, text, reply_to=None):
    ...
    # 1. Hesitação antes de responder (como quem está pensando)
    ...
    if random.random() < COMPLEMENTO_CHANCE:
        ...
        try:
            ...
            await bot.send_message(chat_id=chat_id, text=random.choice(COMPLEMENTOS))
        except Exception:
            pass
            ...
            if response:
                if msg_type == "audio_spontaneous" or (msg_type == "saudade" and random.random() < 0.4): # 40% de chance de saudade ser áudio
                    ...
                    if poll_match:
                        ...
                        await send_humanized(bot, user_id, response)
