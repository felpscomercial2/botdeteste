...
# ==========================================
# Fuso horario do usuario (evita o bot achar que e noite quando ainda e tarde).
TIMEZONE = os.environ.get("TIMEZONE", "America/Sao_Paulo")
try:
    os.environ["TZ"] = TIMEZONE
    _time.tzset()
except Exception:
    logging.warning("Nao consegui aplicar o fuso %s; usando o horario do servidor.", TIMEZONE)
...
CESAR_MOODS = [
    ...
    ("chateado", "Voce ficou meio chateado com algo que ele disse: continua carinhoso, mas mais quieto e direto, e demora um pouco a se soltar."),
]
CESAR_MOOD_HOURS = 6

# ==========================================
# 1.1 Humanização (ritmo, voz, rotina, apelidos)
# ==========================================
# O César NUNCA deixa de responder. O que varia é COMO ele responde.
RESPONDER_SEMPRE = True

# Chance de mandar áudio junto com o texto (antes era sempre -> robótico)
VOICE_CHANCE = float(os.environ.get("VOICE_CHANCE", "0.25"))
# Chance de o César "hesitar" antes de responder (mandar um "hmm", "peraí")
HESITACAO_CHANCE = float(os.environ.get("HESITACAO_CHANCE", "0.05"))
# Chance de escrever errado e se corrigir na mensagem seguinte (bem humano)
TYPO_CHANCE = float(os.environ.get("TYPO_CHANCE", "0.03"))
# Chance de mandar uma segunda mensagem depois de um tempinho (complemento)
COMPLEMENTO_CHANCE = float(os.environ.get("COMPLEMENTO_CHANCE", "0"))

HESITACOES = [
    "hmm", "peraí", "deixa eu pensar", "espera", "hm, calma",
    "então...", "ah, peraí", "opa"
]

COMPLEMENTOS = [
    "esqueci de perguntar: e você, tá bem mesmo?",
    "ah, e eu tava pensando em você agora há pouco",
    "enfim kkk desculpa o textão",
    "voltei, tinha ido pegar um café",
    "sério, me conta mais depois",
]

# Erros de digitação humanos (a palavra certa vem na correção)
TYPOS = {
    "você": "voce", "também": "tambem", "então": "entao", "muito": "muinto",
    "coração": "coracao", "saudade": "sauade", "verdade": "verdaed",
    "amanhã": "amanha", "gosto": "gsoto", "agora": "agoar",
}

# Rotina própria do César (ele tem vida fora do chat)
ROTINA_CESAR = [
    (range(0, 6), "Você provavelmente deveria estar dormindo; se está acordado, é porque ficou mexendo no projeto ou não conseguiu dormir."),
    (range(6, 9), "Você acabou de acordar, tomando café, ouvindo música e organizando o dia."),
    (range(9, 12), "Você está no meio do trabalho: render, planta baixa, cliente mandando áudio."),
    (range(12, 14), "Você parou pra almoçar, geralmente algo rápido em casa."),
    (range(14, 18), "Tarde de trabalho: reunião online, ajustes de projeto, café pela terceira vez."),
    (range(18, 20), "Fim de expediente: você está desligando o computador, talvez indo pra academia ou fazendo janta."),
    (range(20, 23), "Noite: você está no sofá, série ou música rolando, mais relaxado e conversador."),
    (range(23, 24), "Tarde da noite: você está na cama, no celular, com sono mas querendo conversar."),
]

SEMANA_CESAR = {
    5: "É sábado: você está de folga, mais leve, talvez saindo com o Beto ou resolvendo coisas de casa.",
    6: "É domingo: dia calmo, almoço na casa da Dona Marta, preguiça e saudade.",
}

# Apelidos evoluem com a fase do relacionamento
APELIDOS_POR_FASE = {
    "inicio": [],
    "namoro": ["amor", "bebê", "meu bem", "lindo", "meu amor"],
    "casado": ["amor da minha vida", "marido", "meu amor", "bebê", "meu mundo"],
}
...
def init_db():
    ...
    SCHEMA_COLUMNS = {
        "users": [
            ...
            ("open_topic", "TEXT"), ("open_topic_ts", "DATETIME"),
            ("open_topic_asked", "INTEGER DEFAULT 0"),
            ("last_saudade_date", "TEXT"),
        ],
        "relationship_state": [
...
# Peso emocional: o que mais importa aparece primeiro e nunca e cortado.
PESO_CATEGORIA = {
    "data": 5, "pessoa": 5, "medo": 4, "saude": 4, "sonho": 4,
    "trabalho": 3, "desgosto": 2, "gosto": 2, "geral": 1,
}

def format_facts_block(user_id, limit=40):
    todos = get_user_facts_detailed(user_id)
    if not todos:
        return "- (voce ainda nao sabe nada sobre ele. Descubra com perguntas naturais.)"
    # Ordena por peso emocional (mantendo a recencia como desempate natural da consulta)
    todos = sorted(todos, key=lambda r: PESO_CATEGORIA.get(r[2], 1), reverse=True)
    rows = todos[:limit]
    grupos = {}
    for key, value, cat in rows:
        grupos.setdefault(cat, []).append(f"{key.replace('_', ' ')}: {value}")
    ordem = sorted(grupos.keys(), key=lambda c: PESO_CATEGORIA.get(c, 1), reverse=True)
    partes = []
    for cat in ordem:
        peso = PESO_CATEGORIA.get(cat, 1)
        marca = " (MUITO IMPORTANTE, lembre sempre)" if peso >= 4 else ""
        partes.append(f"[{cat.upper()}{marca}]\n" + "\n".join(f"- {i}" for i in grupos[cat]))
    return "\n".join(partes)
...
def get_cesar_mood(user_id):
    ...
    if mood and ts:
        try:
            ...
            fresh = False
    if not fresh:
        # Inercia: o humor nao "reseta" do nada; na maioria das vezes ele so muda um pouco.
        anteriores = [m for m, _ in CESAR_MOODS]
        if mood in anteriores and random.random() < 0.45:
            novo = mood  # continua no mesmo clima
        else:
            candidatos = [m for m in anteriores if m != mood] or anteriores
...
def set_cesar_mood(user_id, mood):
    """Forca um humor do Cesar (ex: chateado depois de uma briga). Dura CESAR_MOOD_HOURS."""
    if mood not in dict(CESAR_MOODS):
        return
    conn = get_db_connection()
    c = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
# ---- Assunto em aberto (continuidade de conversa) ----
GATILHOS_ASSUNTO = [
    "prova", "entrevista", "medico", "médico", "consulta", "exame", "reuniao", "reunião",
    "viagem", "resultado", "problema", "briga", "discussao", "discussão", "projeto",
    "chefe", "trabalho novo", "curriculo", "currículo", "cirurgia", "dentista",
    "mudanca", "mudança", "apresentacao", "apresentação", "aniversario", "aniversário",
]


def registrar_assunto_aberto(user_id, texto):
    """Guarda o ultimo assunto importante para o Cesar retomar depois."""
    t = (texto or "").strip()
    if len(t) < 12 or len(t) > 400:
        return
    if not any(g in t.lower() for g in GATILHOS_ASSUNTO):
        return
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO users (user_id) VALUES (?)", (user_id,))
    c.execute(
        "UPDATE users SET open_topic = ?, open_topic_ts = ?, open_topic_asked = 0 WHERE user_id = ?",
        (t[:300], datetime.now().strftime("%Y-%m-%d %H:%M:%S"), user_id),
    )
    conn.commit()
    conn.close()


def get_assunto_aberto(user_id, min_horas=5):
    """Devolve o assunto pendente se ja passou tempo suficiente e ele ainda nao perguntou."""
    conn = get_db_connection()
    c = conn.cursor()
    try:
        c.execute("SELECT open_topic, open_topic_ts, open_topic_asked FROM users WHERE user_id = ?", (user_id,))
        row = c.fetchone()
    except sqlite3.OperationalError:
        row = None
    conn.close()
    if not row or not row[0] or row[2]:
        return None
    try:
        horas = (datetime.now() - datetime.strptime(row[1], "%Y-%m-%d %H:%M:%S")).total_seconds() / 3600
    except Exception:
        return None
    if horas < min_horas or horas > 96:
        return None
    return row[0], round(horas)


def marcar_assunto_perguntado(user_id):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("UPDATE users SET open_topic_asked = 1 WHERE user_id = ?", (user_id,))
    conn.commit()
    conn.close()


def bloco_saudade_ciume(user_id):
    """Reacao leve quando ele some por muito tempo (no maximo uma vez por dia)."""
    conn = get_db_connection()
    c = conn.cursor()
    try:
        c.execute("SELECT last_interaction, last_saudade_date FROM users WHERE user_id = ?", (user_id,))
        row = c.fetchone()
    except sqlite3.OperationalError:
        conn.close()
        return ""
    ...
    if row[1] == hoje:
        ...
        return ""
    try:
        horas = (datetime.now() - datetime.strptime(row[0], "%Y-%m-%d %H:%M:%S")).total_seconds() / 3600
    except Exception:
        conn.close()
        return ""
    if horas < 10:
        conn.close()
        return ""
    c.execute("UPDATE users SET last_saudade_date = ? WHERE user_id = ?", (hoje, user_id))
    conn.commit()
    conn.close()
    if horas < 24:
        return (
            f"- Ele ficou {int(horas)}h sem falar com voce. Comente isso UMA vez, "
            "com carinho e leveza (senti sua falta / achei que tinha me esquecido), sem drama e sem cobrança pesada.\n"
        )
    return (
        f"- Ele sumiu por {int(horas / 24)} dia(s). Diga que sentiu saudade e pergunte se esta tudo bem, "
        "com um toque de manha carinhosa. Nunca faca chantagem emocional nem fique repetindo a cobranca.\n"
    )
...
# ---- Descoberta automatica de modelos de visao na Groq ----
# Os nomes dos modelos mudam/saem do ar com frequencia. Em vez de depender de uma
# lista fixa (que quebrava a leitura de fotos), consultamos a propria API.
_VISION_CACHE = {"models": [], "ts": 0.0}
_VISION_CACHE_TTL = 1800  # 30 minutos
_VISION_HINTS = ("scout", "maverick", "vision", "llava", "vl", "omni")
_VISION_BLOCK = ("whisper", "tts", "guard", "embedding", "reranker")


def _retry_after(response, tentativa):
    """Respeita o Retry-After do provedor; senao usa backoff exponencial suave."""
    try:
        valor = float(response.headers.get("retry-after", ""))
        return max(1.0, min(valor, 30.0))
    except Exception:
        return min(2 ** tentativa * 2, 20)


async def telegram_call_with_retry(operation, *, attempts=4, operation_name="Telegram"):
    """Repete operacoes idempotentes do Telegram durante oscilacoes breves de rede."""
    last_error = None
    for tentativa in range(attempts):
        try:
            return await operation()
        except (NetworkError, TelegramError, httpx.RequestError, asyncio.TimeoutError) as error:
            last_error = error
            if tentativa == attempts - 1:
                break
            delay = min(1.5 * (2 ** tentativa), 8.0) + random.uniform(0.0, 0.4)
            logging.warning(
                "%s falhou por conexao (tentativa %s/%s): %s; repetindo em %.1fs",
                operation_name, tentativa + 1, attempts, error, delay,
            )
            await asyncio.sleep(delay)
    if last_error is not None:
        raise last_error
    raise RuntimeError(f"{operation_name} falhou sem informar o motivo")


async def send_telegram_message(bot, chat_id, text):
    """Envia uma bolha sem fazer o usuario repetir a mensagem por uma falha temporaria."""
    return await telegram_call_with_retry(
        lambda: bot.send_message(chat_id=chat_id, text=text),
        operation_name="Envio da mensagem",
    )


async def download_telegram_bytes(bot, file_id):
    """Baixa uma midia novamente quando a conexao com o Telegram oscila."""
    async def _download():
    ...
    return await telegram_call_with_retry(
        _download,
        attempts=3,
        operation_name="Download da midia",
    )


async def modelos_de_visao(force=False):
    """Lista de modelos capazes de ler imagens, com cache e fallback."""
    agora = _time.time()
    if not force and _VISION_CACHE["models"] and agora - _VISION_CACHE["ts"] < _VISION_CACHE_TTL:
        return _VISION_CACHE["models"]
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                descobertos = [
                    mid for mid in ids
                    if mid
                    and any(h in mid.lower() for h in _VISION_HINTS)
                    and not any(b in mid.lower() for b in _VISION_BLOCK)
                ]
                logging.info("Modelos de visao descobertos na Groq: %s", descobertos)
            else:
                logging.warning("Nao consegui listar modelos da Groq: %s %s", r.status_code, r.text[:200])
        except Exception as e:
            logging.warning("Falha ao listar modelos da Groq: %s", e)

    ordenados = []
    # Primeiro os configurados manualmente que realmente existem, depois os descobertos.
    for mid in VISION_MODELS:
        if not descobertos or mid in descobertos:
            ordenados.append(mid)
    for mid in descobertos:
        if mid not in ordenados:
            ordenados.append(mid)
    if not ordenados:
        ordenados = list(VISION_MODELS)

    _VISION_CACHE["models"] = ordenados
    _VISION_CACHE["ts"] = agora
    return ordenados
...
async def analyze_image_with_vision(image_source, prompt="Descreva com detalhes o que aparece nesta imagem (pessoas, roupas, lugar, objetos, clima, expressao), para que eu possa comentar de forma pessoal."):
    ...
    modelos = await modelos_de_visao()
    if not modelos:
        logging.error("Nenhum modelo de visao disponivel na conta Groq.")
        return None
        ...
        for model in modelos[:2]:
            for tentativa in range(3):
                try:
                    ...
                    corpo = response.text[:400]
                    logging.warning(f"Visao {model} status {response.status_code}: {corpo}")
                    if response.status_code in (400, 404) and (
                        "decommission" in corpo.lower() or "does not exist" in corpo.lower()
                        or "model_not_found" in corpo.lower()
                    ):
                        # Modelo saiu do ar: invalida o cache para redescobrir na proxima foto
                        globals()["_VISION_CACHE"] = {"models": [], "ts": 0.0}
                        break
                    if response.status_code in (429, 500, 502, 503, 504):
                        espera = _retry_after(response, tentativa)
    ...
    logging.error("Todos os modelos de visao falharam. Modelos tentados: %s", modelos)
    return None
...
async def get_groq_response(user_id, user_text, retry_count=0):
    ...
    # handle_message salva a mensagem antes de chamar esta função. Remova essa
    # última cópia para que a fala atual seja enviada ao modelo exatamente uma vez.
    ...
    # Continuidade: retomar assunto que ficou em aberto
    _assunto = get_assunto_aberto(user_id)
    if _assunto:
        _txt, _horas = _assunto
        humor_bloco += (
            "ASSUNTO EM ABERTO (retome com naturalidade, como quem se importou de verdade):\n"
            f"- Há cerca de {_horas}h ele te contou: \"{_txt}\"\n"
            "- Pergunte como foi/como ficou isso em UMA frase, sem soar interrogatório, e siga a conversa.\n\n"
        )
        marcar_assunto_perguntado(user_id)
    # Saudade/ciume leve quando ele some
    _saudade = bloco_saudade_ciume(user_id)
    if _saudade:
        humor_bloco += "SAUDADE (use uma vez só, com leveza):\n" + _saudade + "\n"
    ...
    system_prompt = (
        ...
        "COMO SOAR HUMANO DE VERDADE (leia com atenção):\n"
        "- VARIE O TAMANHO E O TOM: alterne entre respostas de uma palavra ('kkkk', 'sério?', 'nossa'), "
        "respostas médias e, de vez em quando, um textão empolgado. Nunca tenha um formato fixo.\n"
        "- VARIE A VOZ: às vezes brincalhão, às vezes sério, às vezes carinhoso, às vezes de birra fofa, "
        "às vezes cansado. Gente real muda de humor dentro da mesma conversa.\n"
        "- HESITE COMO GENTE: pode começar com 'hmm', 'deixa eu pensar', 'pera', 'sei lá', 'acho que...'. "
        "Pode mudar de ideia no meio da frase e se corrigir.\n"
        "- ESCREVA COMO NO CELULAR: minúsculas às vezes, 'tô', 'né', 'pra', 'vc' de vez em quando, "
        "reticências, risada variada (kkk, kkkkkk, haha, rsrs). Nunca escreva como redação.\n"
        "- RETOME ASSUNTOS: volte a algo que ele falou antes ('lembrei do que você falou ontem sobre...'). "
        "Isso é o que mais faz parecer gente de verdade.\n"
        "- FAÇA PERGUNTAS DE ACOMPANHAMENTO: se ele disse que tinha reunião, pergunte depois como foi. "
        "Se ele estava doente, pergunte se melhorou.\n"
        f"- VOCÊ TEM VIDA PRÓPRIA AGORA: {get_rotina_cesar()} Conte pedaços disso espontaneamente, "
        "sem transformar em relatório. Às vezes você demora porque estava ocupado — comente com naturalidade.\n"
        "- EMOJIS E STICKERS COM INTENÇÃO: use só quando reforçam o que você está sentindo, nunca de enfeite.\n"
        + (
            "- APELIDOS: nesta fase você pode chamá-lo de "
            + ", ".join(get_apelidos_permitidos(fase, namoro, casamento))
            + ", variando (não repita sempre o mesmo).\n"
            if get_apelidos_permitidos(fase, namoro, casamento) else
            "- APELIDOS: ainda não use apelidos carinhosos, só o nome dele.\n"
        )
        + (
            "- DESPEDIDAS COM SAUDADE: quando ele disser que vai sair/trabalhar/dormir, demonstre que vai "
            "sentir falta, peça pra ele avisar quando voltar.\n" if namoro else
            "- DESPEDIDAS: quando ele for sair, desejar um bom dia/boa noite com simpatia, sem drama.\n"
        )
        + "- REGRA INEGOCIÁVEL: você SEMPRE responde. Nunca ignore, nunca deixe no vácuo, nunca diga que "
        "vai responder depois sem responder agora. Mesmo ocupado, você responde."
    )
    ...
    regras_finais = (
        ...
        "7. Responda PRIMEIRO e diretamente à última fala do usuário. Não troque de assunto, "
        "não faça uma pergunta aleatória e não retome assunto antigo nesta resposta.\n"
        "8. Faça no máximo UMA pergunta, e somente se ela tiver relação direta com a última fala.\n"
    )
          ...
          for tentativa in range(3):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code in (429, 500, 502, 503, 504):
                        await asyncio.sleep(_retry_after(response, tentativa))
                        continue
                    break
            except Exception as e:
    ...
    # A mensagem original ja esta salva. Nunca transfere ao usuario a tarefa de repeti-la.
    return (
        "A conexão oscilou aqui e eu não consegui terminar minha resposta agora. "
        "Mas guardei o que você disse — pode continuar, que eu retomo daqui sem você repetir.",
        "neutro",
    )
...
def get_rotina_cesar():
    """O que o César está fazendo agora (ele tem vida própria)."""
    agora = datetime.now()
    linha = ""
    for faixa, desc in ROTINA_CESAR:
        if agora.hour in faixa:
            linha = desc
            break
    extra = SEMANA_CESAR.get(agora.weekday(), "")
    return " ".join([p for p in (linha, extra) if p])


def get_apelidos_permitidos(fase, namoro, casamento):
    if casamento:
        return APELIDOS_POR_FASE["casado"]
    if namoro:
        return APELIDOS_POR_FASE["namoro"]
    return APELIDOS_POR_FASE["inicio"]


def _aplicar_typo(texto):
    """Troca uma palavra por uma versão com erro; devolve (texto_errado, correcao)."""
    for certo, errado in TYPOS.items():
        if re.search(rf"\b{certo}\b", texto, flags=re.IGNORECASE):
            novo = re.sub(rf"\b{certo}\b", errado, texto, count=1, flags=re.IGNORECASE)
            return novo, f"*{certo}"
    return texto, None


def quebrar_em_bolhas(text):
    """Divide a fala em mensagens curtas, como gente digitando no Telegram."""
    partes = [p.strip() for p in re.split(r"\n{2,}", text) if p.strip()]
    if len(partes) == 1 and len(text) > 140:
        frases = re.split(r"(?<=[.!?…])\s+", text.strip())
        if len(frases) > 2:
            corte = max(1, len(frases) // random.choice([2, 3]))
            partes = []
            atual = []
            for f in frases:
                atual.append(f)
    ...
    # Nunca descartar conteúdo: o excedente é juntado na última bolha,
    # senão o áudio (que lê o texto todo) falaria mais do que foi escrito.
    if len(partes) > 3:
        partes = partes[:2] + [" ".join(partes[2:]).strip()]
    return partes


async def send_humanized(bot, chat_id, text, reply_to=None):
    """Envia a resposta com ritmo humano: hesitação, digitação, bolhas, typos e complemento."""
    if not text:
        return ""

    if random.random() < HESITACAO_CHANCE:
        try:
            await bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
            await asyncio.sleep(random.uniform(0.8, 1.8))
            await send_telegram_message(bot, chat_id, random.choice(HESITACOES))
            await asyncio.sleep(random.uniform(0.8, 2.0))
        except Exception:
            pass

    partes = quebrar_em_bolhas(text)
    typo_pendente = None
    enviadas = []

    for i, parte in enumerate(partes):
        # 2. Erro de digitação humano, corrigido logo depois
        if typo_pendente is None and random.random() < TYPO_CHANCE:
            parte, typo_pendente = _aplicar_typo(parte)
        ...
        # 3. Velocidade de digitação variável (às vezes ele digita rápido, às vezes devagar)
        velocidade = random.uniform(22, 55)
        pausa = min(7.0, 0.7 + len(parte) / velocidade)
        if len(parte) < 25:
            pausa = random.uniform(0.6, 1.8)
        await asyncio.sleep(pausa)

        try:
            await send_telegram_message(bot, chat_id, parte)
            enviadas.append(parte)
        except Exception as e:
        ...
        if typo_pendente:
            ...
            try:
                await send_telegram_message(bot, chat_id, typo_pendente)
            except Exception:
                pass
            typo_pendente = None
    ...
    # 4. Às vezes ele volta com um complemento, como quem lembrou de algo
    if random.random() < COMPLEMENTO_CHANCE:
        await asyncio.sleep(random.uniform(3.0, 8.0))
        try:
            await bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
            await asyncio.sleep(random.uniform(1.0, 2.0))
            complemento = random.choice(COMPLEMENTOS)
            await send_telegram_message(bot, chat_id, complemento)
            enviadas.append(complemento)
        except Exception:
            pass

    # Devolve exatamente o que foi escrito, para o áudio falar a mesma coisa.
    if not enviadas:
        raise RuntimeError("Nenhuma bolha de resposta foi enviada ao Telegram")
    return "\n\n".join(enviadas).strip()
...
def fazer_backup_local(manter=7):
    """Copia o banco para a pasta de midia e mantem apenas os N backups mais recentes."""
    if not MEDIA_DIR:
        return None
    try:
        pasta = os.path.join(MEDIA_DIR, "backups")
        os.makedirs(pasta, exist_ok=True)
        destino = os.path.join(pasta, f"bot_memory_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db")
        ...
        for velho in arquivos[manter:]:
            try:
                ...
                pass
        logging.info("Backup automatico criado: %s", destino)
        return destino
    except Exception as e:
        logging.error("Falha no backup automatico: %s", e)
        return None


async def backup_job(application=None):
    await asyncio.to_thread(fazer_backup_local)
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mostra fase do relacionamento, afinidade e humor atual do Cesar."""
    user_id = update.effective_user.id
    try:
        fase, points, segredo, namoro, noivado, casamento = get_relationship_status(user_id)[:6]
    except Exception:
        estado = get_relationship_status(user_id)
        fase, points = estado[0], estado[1]
        segredo = namoro = noivado = casamento = 0
    mood, desc = get_cesar_mood(user_id)
    n_fatos = len(get_user_facts_detailed(user_id))
    marcos = []
    if namoro: marcos.append("namoro")
    if segredo: marcos.append("segredo contado")
    if noivado: marcos.append("noivado")
    if casamento: marcos.append("casamento")
    texto = (
        "Como a gente tá hoje:\n"
        f"• Fase: {fase}\n"
        f"• Afinidade: {points} pontos\n"
        f"• Meu humor: {mood} — {desc}\n"
        f"• Coisas que sei sobre você: {n_fatos}\n"
        f"• Marcos: {', '.join(marcos) if marcos else 'ainda nenhum'}"
    )
...
async def reset_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for tabela in tabelas:
        try:
            c.execute(f"DELETE FROM {tabela} WHERE user_id = ?", (user_id,))
        except Exception as e:
            logging.warning("Nao consegui limpar %s no reset: %s", tabela, e)
    conn.commit()
    conn.close()

    msg = f"Apaguei tudo o que eu sabia sobre você. Memória zerada! Vamos começar do zero. Oi, tudo bem? Sou o César, mas pode me chamar de {BOT_NICKNAME}."
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Guarda assunto importante para retomar depois ("e aquela entrevista?")
    try:
        registrar_assunto_aberto(user_id, update.message.text or update.message.caption or "")
    except Exception as e:
        logging.warning(f"Nao consegui registrar o assunto aberto: {e}")

    # Briga/desaforo: o humor do Cesar fica chateado por algumas horas (inercia emocional)
    if any(w in text_lower for w in ["chato", "me deixa", "cala a boca", "idiota", "burro", "some", "odeio voce", "odeio você"]):
        try:
            set_cesar_mood(user_id, "chateado")
        except Exception:
            pass
    ...
    # ---- FOTO recebida do usuário ----
        ...
        try:
            ...
            # Baixa em memoria e tenta novamente se a conexao do Telegram oscilar.
            photo_bytes = await download_telegram_bytes(context.bot, photo.file_id)
            if not photo_bytes:
    ...
    # ---- DOCUMENTO ----
        ...
        await context.bot.send_chat_action(chat_id=user_id, action=ChatAction.TYPING)
        try:
            doc_bytes = await download_telegram_bytes(context.bot, update.message.document.file_id)
    ...
    try:
        ...
        texto_enviado = await send_humanized(context.bot, user_id, response_text)
        # So registra como fala do Cesar depois que o Telegram confirmou o envio.
        await db(save_message, user_id, "model", texto_enviado)
        # Áudio ocasional (gente não manda áudio o tempo todo), mais provável
        # quando a emoção é forte ou quando ele mandou áudio primeiro.
        chance_audio = VOICE_CHANCE
        if emotion in ("feliz", "empolgado", "triste", "preocupado"):
            chance_audio += 0.15
        if update.message.voice or update.message.audio:
            chance_audio = 1.0
        if random.random() < chance_audio:
            await generate_voice(context.bot, user_id, texto_enviado or response_text, emotion)
        ...
        try:
            await send_telegram_message(
                context.bot,
                user_id,
                "A conexão oscilou, mas sua mensagem ficou salva. Não precisa repetir — pode continuar daqui.",
            )
        except Exception as send_error:
            logging.error("Tambem nao consegui enviar o aviso de recuperacao: %s", send_error)
    finally:
        processing_users.discard(user_id)
...
async def check_spontaneous(application, forced_type=None):
    ...
    for user_id in user_ids:
        ...
        if not forced_type and tempo_sem_interagir.total_seconds() < (TEMPO_SENTIR_FALTA * 3600):
            # Quanto maior o silêncio, maior a vontade dele de puxar assunto do nada
            horas_quietas = tempo_sem_interagir.total_seconds() / 3600
            chance_iniciativa = min(0.35, 0.08 + horas_quietas * 0.06)
            if random.random() < chance_iniciativa:
                early_types = ["lembrete", "project", "poll_project", "virtual_gift", "portfolio_project"]
                if namoro:
            ...
            if response:
                if msg_type == "audio_spontaneous" or (msg_type == "saudade" and random.random() < 0.4): # 40% de chance de saudade ser áudio
                    ...
                    if poll_match:
                        ...
                        if intro_text:
                            await send_humanized(bot, user_id, intro_text)
                        ...
                        enviado = await send_humanized(bot, user_id, response)
                        if random.random() < VOICE_CHANCE:
                            await generate_voice(bot, user_id, enviado or response)
                        save_message(user_id, "model", response)
                else:
                    enviado = await send_humanized(bot, user_id, response)
                    if random.random() < VOICE_CHANCE:
...
async def post_init(application):
    ...
    # Job 7: Backup automatico diario do banco (4h da manha), mantendo os 7 ultimos
    scheduler.add_job(
        backup_job,
    ...
    try:
        ...
        if chat_id:
            await send_telegram_message(
                context.bot,
                chat_id,
                "Tive uma oscilação aqui, mas guardei sua mensagem. Não precisa mandar de novo.",
            )
    except Exception:
        pass
