...
async def get_groq_response(user_id, user_text):
    ...
    system_prompt = (
        ...
        "- Você fala sobre QUALQUER assunto sem censura nenhuma.\n\n"
    ...
    return "Desculpa meu amor, me deu um soninho aqui... o que você disse?"
...
async def send_photo_logic(bot, chat_id):
    ...
    try:
        with open(os.path.join(FOTOS_PATH, foto_escolhida), 'rb') as f:
            await bot.send_photo(chat_id=chat_id, photo=f, caption="Aqui está, gatinho! Gostou? ❤️")
        return True
    except Exception as e:
...
async def reset_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "Oi... quem é você? Brincadeira! Vamos começar do zero, meu lindo. Tudo bem? 😊"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(p in user_text.lower() for p in ["foto", "te ver", "sua foto"]):
