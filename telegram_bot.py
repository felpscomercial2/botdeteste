...
def save_health_reminder(user_id, ailment):
    conn = get_db_connection()
    c = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute("INSERT OR REPLACE INTO health_reminders (user_id, ailment, reported_timestamp) VALUES (?, ?, ?)", (user_id, ailment, now))
    conn.commit()
    conn.close()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(w in text_lower for w in ["triste", "mal", "ruim", "péssimo", "chorando", "difícil"]):
        ...
        save_health_reminder(user_id, "estresse")
        ...
        save_health_reminder(user_id, "dor de cabeça")
        ...
        save_health_reminder(user_id, "doente")
    ...
    # ---- FOTO recebida do usuário ----
        ...
        try:
            ...
            if update.message.caption:
                user_text = f"[mandou uma foto. Descrição do que você vê: {vision_description}. Legenda do usuário: {update.message.caption}]"
            else:
                user_text = f"[mandou uma foto. Descrição do que você vê: {vision_description}]"
            user_text += " O Lucas deve responder com carinho, elogiando detalhes específicos da foto que ele 'viu'."
            ...
            user_text = "[mandou uma foto, mas não consegui ver os detalhes agora] O Lucas deve responder com carinho."
    ...
    if spotify_match:
        ...
        if spotify_info:
            user_text += f" [O link do Spotify é sobre: {spotify_info}. O Lucas deve comentar sobre essa música/playlist, dizer se gostou ou o que sentiu.]"
        else:
...
async def check_spontaneous(application, forced_type=None):
    ...
    for user_id in user_ids:
        ...
        # Verificar lembretes de saúde ativos
...
async def generate_spontaneous_message(user_id, horas_separados, type="saudade"):
