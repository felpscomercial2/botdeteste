...
def init_db():
    ...
    ''')
    # Colunas do humor persistente do Cesar
    for col, ddl in (("cesar_mood", "TEXT"), ("cesar_mood_timestamp", "DATETIME")):
        try:
...
async def analyze_image_with_vision(image_path, prompt="Descreva com detalhes o que aparece nesta imagem (pessoas, roupas, lugar, objetos, clima, expressao), para que eu possa comentar de forma pessoal."):
    """Analisa a foto de verdade: reduz a imagem, tenta varios modelos e loga o erro real."""
    ...
    try:
        ...
        logging.error(f"Nao consegui abrir a foto {image_path}: {e}")
        return None

    # Groq limita imagens base64 (~4MB). Reduzir evita falha silenciosa.
    if Image is not None:
        try:
            ...
            logging.warning(f"Nao consegui redimensionar a foto, seguindo com a original: {e}")
...
async def get_groq_response(user_id, user_text, retry_count=0):
    ...
    system_prompt = (
        ...
        "IDENTIDADE DO LUCAS:\n"
        f"- Você é o César, um homem gay de 30 anos. {nome_clause}\n"
        f"{weather_info}"
        f"{sleep_info}\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- FOTO recebida do usuário ----
        ...
        try:
            ...
            user_text = (
                "[O usuário mandou uma foto e a análise de imagem falhou agora. "
                f"Legenda dele: {legenda}. Reaja com naturalidade: diga que a foto demorou a carregar no seu celular "
                "e peça para ele mandar de novo ou contar o que é. NUNCA invente detalhes que você não viu.]"
            )
        finally:
            try:
                if 'photo_path' in dir() and os.path.exists(photo_path):
