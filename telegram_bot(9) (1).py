...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_chat_action(chat_id=user_id, action=ChatAction.RECORD_AUDIO)
...
async def reset_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute("DELETE FROM history WHERE user_id = ?; DELETE FROM users WHERE user_id = ?; DELETE FROM facts WHERE user_id = ?", (user_id, user_id, user_id))
    conn.commit(); conn.close()
...
async def main():
    ...
    # O scheduler precisa ser iniciado dentro do loop de eventos rodando
    scheduler = AsyncIOScheduler(timezone=BR_TZ)
    scheduler.add_job(check_saudade, 'interval', minutes=30, args=[app])
    scheduler.start()
