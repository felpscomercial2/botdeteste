...
async def generate_voice(bot, chat_id, text):
    """Gera e envia áudio com fallback de voz e retry."""
    ...
    for voz in [VOICE_PRIMARY, VOICE_SECONDARY]:
        try:
            logging.info(f"TTS: gerando áudio com voz {voz} para chat {chat_id}")
            communicate = edge_tts.Communicate(clean_text, voz, rate=RATE)
...
if __name__ == '__main__':
    ...
    # Iniciar o scheduler ANTES do bot
    scheduler.start()
    print("Scheduler de tarefas agendadas iniciado.")
    
    # Configurar aplicação com post_init para registrar os jobs
    application = (
        ApplicationBuilder()
